...
mod tests {
    ...
    impl TestContext {
        async fn init() -> Self {
            ...
            let mut stash: PairSigner<ContractsTemplateRuntime, Pair> = PairSigner::new(AccountKeyring::Alice.pair());
            let nonce = client
            ...
            TestContext { client, signer, requester }
        }
            ...
            let gas: Gas = 500_000;
            let result = self.client.multistep_call_and_watch(
                &self.signer,
                requester,
                target_dest,
            ...
            let execution_success_event = result.multistep_execute_phase_success()?.ok_or_else(|| {
                Error::Other("Failed to find a MultistepExecutePhaseSuccess event".into())
            })?;
            log::info!("MultistepExecutePhaseSuccess execution_stamp: {:?}", execution_success_event.execution_stamp);
